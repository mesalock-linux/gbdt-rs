use super::decision_tree::{ValueType, DataVec, PredVec};
...
pub fn AUC(dv: &DataVec, predict: &PredVec, len: usize) -> ValueType {
    ...
    for i in 0..dv.len() {
        if dv[i].label > 0.0 {
            ...
            if predict[i] >= threshold { confusion_table[3] += 1 }
            else { confusion_table[2] += 1 }
        } else {
            negative_scores.push(predict[i]);
            if predict[i] >= threshold { confusion_table[1] += 1 }
            else { confusion_table[0] += 1 }
        }
    }
    ...
    if (p_size == 0) || (n_size == 0) { return 0.5; }
...
pub fn weighted_label_median(dv: &DataVec, len: usize) -> ValueType {
    ...
    for d in &dv_copy { all_weight += d.weight; }
